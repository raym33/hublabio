...
impl Version {
    ...
    pub fn parse(s: &str) -> Option<Self> {
        ...
        Some(Self {
            ...
            build: parts.iter().find(|p| p.chars().all(|c| c.is_ascii_digit()))
                .and_then(|s| s.parse().ok()),
            prerelease: parts.get(1).filter(|p| !p.chars().all(|c| c.is_ascii_digit()))
                .map(|s| s.to_string()),
        })
    }
...
impl UpdateManager {
    ...
    pub fn download(&mut self) -> Result<(), UpdateError> {
        let manifest = self.available_update.as_ref()
    ...
    pub fn install(&mut self) -> Result<(), UpdateError> {
        if self.state != UpdateState::Verifying {
            return Err(UpdateError::InstallFailed(String::from("Not ready to install")));
        }
    ...
    pub fn reboot(&self) -> Result<(), UpdateError> {
        if self.state != UpdateState::PendingReboot {
            return Err(UpdateError::InstallFailed(String::from("No update pending")));
        }
    ...
    pub fn status_summary(&self) -> String {
        ...
        if let Some(update) = &self.available_update {
            ...
            output.push_str(&format!("Download size: {} MB\n", update.download_size / (1024 * 1024)));
        }
    ...
    fn format_slot(&self, slot: &PartitionInfo) -> String {
        let version = slot.version.as_ref()
            .map(|v| v.to_string())
