...
impl SyscallTracer {
    ...
    pub fn format_syscall(&self, entry: &SyscallEntry) -> String {
        ...
        output.push_str(&format!("{}({}) = ", entry.name, args_str.join(", ")));
...
fn parse_open_flags(flags: u64) -> Vec<String> {
    ...
    if flags & 0x40 != 0 {
        names.push(String::from("O_CREAT"));
    }
    if flags & 0x80 != 0 {
        names.push(String::from("O_EXCL"));
    }
    if flags & 0x200 != 0 {
        names.push(String::from("O_TRUNC"));
    }
    if flags & 0x400 != 0 {
        names.push(String::from("O_APPEND"));
    }
    if flags & 0x800 != 0 {
        names.push(String::from("O_NONBLOCK"));
    }
