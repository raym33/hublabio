...
impl Debugger {
    ...
    pub fn execute_command(&mut self, cmd: &str) -> Result<String, DebugError> {
        ...
        match parts[0] {
            ...
            "info" => match parts.get(1).unwrap_or(&"") {
                &"breakpoints" | &"b" => {
                ...
                &"registers" | &"r" => Ok(self.registers.format()),
            ...
            "print" | "p" => {
                ...
                if name.starts_with('$') {
                    ...
                    if let Some(sym) = self.lookup_symbol(self.parse_address(name).unwrap_or(0)) {
