...
impl HtmlRenderer {
        ...
        match node {
            ...
            DomNode::Element { tag, attrs, children } => {
                ...
                match tag_lower.as_str() {
                    ...
                    "tr" => {
                        let cells: Vec<String> = children.iter()
                            .filter_map(|c| {
                            ...
                            .collect();
                        self.add_line(&format!("│ {} │", cells.join(" │ ")), false, 0, false, None, false);
                    }

                    // Forms (show input placeholders)
                    "input" => {
                        let placeholder = attrs.get("placeholder").map(|s| s.as_str()).unwrap_or("");
                        let input_type = attrs.get("type").map(|s| s.as_str()).unwrap_or("text");
                        self.add_line(&format!("[{}: {}]", input_type, placeholder), false, 0, false, None, false);
                    }
    ...
    fn add_line(&mut self, text: &str, is_heading: bool, heading_level: u8, is_link: bool, link_url: Option<String>, is_list_item: bool) {
